...
class ContentPackTest {
    ...
    private fun validateQuestion(q: Question, ids: MutableSet<String>, where: String) {
        ...
        when (q) {
            ...
            is Question.Math -> Unit   // 수학 전용 앱 — 수학 문제가 정상
            is Question.Match -> {
    ...
    @Test fun everyQuestionHasKnownSkill() {
        ...
        for (tid in com.piyak.english.model.Subject.MATH.tracks) {
            ...
            for (u in t.units) for (l in u.lessons) for (q in l.questions) {
                ...
                assertTrue("$tid/${q.id}: 수학 팩에 영어 영역이 섞임", q.skill !in engSkills)
    ...
    @Test fun placementPoolValid() {
        ...
        for (i in 0 until arr.length()) {
            ...
            assertTrue("레벨 범위", lv in 1..13)
            val q = Question.fromJson(o)
            // 수학 배치고사는 빠르게 넘겨야 해서 선다형이거나 숫자 답이어야 한다
            assertTrue("배치고사 문제 타입", q is Question.Math)
